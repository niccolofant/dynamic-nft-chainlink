...
task("create-metadata", "Create metadata for all created characters")
  ...
  .setAction(async (taskArgs) => {
    ...
    const dungeonsDragonsContract = await ethers.getContractAt(
    ...
    while (index < length) {
      ...
      characterMetadata["attributes"][0]["value"] =
        characterOverview["strength"].toNumber();
      characterMetadata["attributes"][1]["value"] =
        characterOverview["dexterity"].toNumber();
      characterMetadata["attributes"][2]["value"] =
        characterOverview["constitution"].toNumber();
      characterMetadata["attributes"][3]["value"] =
        characterOverview["intelligence"].toNumber();
      characterMetadata["attributes"][4]["value"] =
        characterOverview["wisdom"].toNumber();
      characterMetadata["attributes"][5]["value"] =
        characterOverview["charisma"].toNumber();
      filename =
        "metadata/" +
        characterMetadata["name"].toLowerCase().replace(/\s/g, "-");
      ...
      console.log(
        "npx hardhat set-uri --contract " +
          contractAddr +
          " --tokenid InsertIdHere" +
          " --uri insertURIHere" +
          " --network " +
          network.name
      );
