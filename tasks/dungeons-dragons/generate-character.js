task("generate-character", "Generate a provably random Chainlink D&D Character")
  ...
  .setAction(async (taskArgs) => {
    ...
    console.log(
      "Generating a provably random Chainlink D&D Character ",
      contractAddr,
      " on network ",
      networkId
    );
    const DungeonsDragons = await ethers.getContractFactory(
    ...
    const dungeonsDragonsContract = new ethers.Contract(
      contractAddr,
      DungeonsDragons.interface,
      signer
    );
    const transactionResponse =
      await dungeonsDragonsContract.requestNewRandomCharacter(taskArgs.name);
    const transactionReceipt = await transactionResponse.wait();
    console.log(
      "Contract ",
      contractAddr,
      " random character generated. Transaction Hash: ",
      transactionResponse.hash
    );
    console.log(
      "Run the following to generate the metadata for created characters:"
    );
    console.log(
      "npx hardhat create-metadata --contract " +
        contractAddr +
        " --network " +
        network.name
    );
